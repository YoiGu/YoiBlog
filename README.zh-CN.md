...
- **文章编辑器** — 分栏式 Markdown 编辑器，左侧编写，右侧实时预览。预览使用与生成器相同的渲染管线，所见即所得——包括数学公式、语法高亮代码块和所有扩展语法。支持直接上传 `.md` 文件（自动解析 YAML front matter 并填充标题、标签、分类、摘要等字段），也可以从零开始编写。
- **文章列表** — 所有文章以可排序的表格展示，点击列标题按标题、日期或状态排序。支持确认后删除。
- **站点预览** — 在 WebUI 内启动本地预览服务器，通过内嵌 iframe 查看完整生成的站点效果。支持启动/停止、重新构建和在新标签页中打开。
...
- **明暗模式** — WebUI 支持明暗主题切换，切换按钮位于侧栏底部，偏好持久化到 `localStorage`。视觉设计与博客默认主题统一——相同的渐变色调、字体排版和强调色。
...
└── webui/
    ...
    ├── routes.py        # 页面路由（仪表盘、编辑器、文章列表、设置、部署、预览）
    ├── preview_server.py # 后台预览服务器管理器（启动/停止/重建）
    ├── i18n.py          # 完整的中英文翻译字典
    ├── static/          # WebUI 专用 CSS 和静态资源
    └── templates/       # WebUI Jinja2 模板（7 个页面）
```
